...
상태: 5판 + V0 결과 (2026-09-21 닫힘. 코덱스 최종 검토 판정 "설계 전제 검증: 아니오", 권고 ③ 멈춤)
...
코덱스가 저장소 커밋 `9372d5b`에서 코드를 직접 돌려 검토했다. 전문: `docs/reviews/2026-09-21-codex-final.md`.

**판정: "이 V0가 설계 전제를 검증했다" — 아니오. 권고: ③ 여기서 멈춤.**
프로젝트 폐기가 아니다. "추출 결함을 발견하고 현재 규칙의 문자 적중 분포를 확인한 탐색 실행"으로 보존한다.

### 쓸 수 있는 문장 (코덱스가 준 그대로)

> 현재 v0.1.0의 문자열 규칙은 추출기가 받아들인 276개 프롬프트 중 179개에서 어떤 작업 종류 어휘도
> 찾지 못했다. 작업 적합성과 추출 정확도는 미검증이다.

**쓸 수 없는 문장:** "규칙만으로는 부족하다." 분모가 검증된 작업이 아니라 추출기가 받아들인 전부이고,
사실 정정·동의·연속 요청이 섞여 있다. 65%는 규칙 방식의 한계가 아니라 **현 규칙의 문자 적중 범위**다.

### 재현된 수치 [MEASURED, 코덱스 독립 재실행과 일치]

| 항목 | 값 |
|---|---|
| user 이벤트 → accepted | 3,038 → 276 |
| 작업 종류 MATCHED / AMBIGUOUS / NO_MATCH | 68 / 29 / 179 (25 / 11 / 65%) |
| 도메인 evidence 적중 | **9 / 276 = 3.3%** (개념 제목 1 · AGENTS 절 0 · 폴더명 9, 중복 포함) |
| 추가 prefix 필터 제외 | 110건 (Base directory 19 · Another Claude session 83 · Request interrupted 8) |

**정정.** 최종 검토 요청문에 "도메인 어휘 적중 19%"라고 적었다. 그것은 추출기 수정 **전** run(74/386)의
값이고 수정 후 재측정 없이 옮겨 적은 오류다. 이 세션에서 같은 종류의 오류가 세 번째다
(14/91 → 9/36, 330→86 재현 실패, 19% → 3.3%). 세 번 다 **"수치 옆에 실행 명령"** 규칙을 어긴 자리였다.
"28%가 시스템 문장"도 "386 중 110건이 prefix 필터로 제외됨"이 정확하다. 110건 각각의 판정은 검증하지 않았다.

### 알려진 결함 (고치지 않았다 — 멈춤 권고에 따라 기록만)

우선 수정으로 표시된 셋. 후속 실험을 시작하면 첫 커밋에서 고친다.

1. **실행 번호와 evaluation_status가 코드에 없다.** `score`는 항상 `run #1 · BASELINE`을 쓰고 같은 파일을
   덮어쓴다. 저장된 run #2 표제는 손으로 고친 것. 5판의 실행 보존 계약이 미구현.
2. **지문이 내용을 보호하지 않는다.** corpus 해시는 행 id만, manifest 해시는 경로+크기만. 본문을 바꿔도 같다.
   cutoff도 실행마다 바뀌고 고정 인자가 없다.
3. **버린 행의 원문이 없다.** rejected 2,762건 모두 `text` 없음. 0-A의 "버린 쪽 검증"이 라벨 화면에서 불가능.

그 외: `<`로 시작하면 전부 버림(`<button> 만들어줘`도) · 부분 문자열 매칭("왜냐하면"→question) · 대소문자
민감(`README`≠`readme`) · STEP 2 라벨 경로 없음 · accepted 0이면 0 나눗셈 · concepts 파서 실패를 빈 목록과
구분 못 함 · 예시 5건은 표본이 아니라 처음 만난 것 · 폴더명·프롬프트 비교에 NFC 정규화 없음.

### 개념 지도에 대해

3.3%는 개념 지도의 효용을 기각하는 수치가 아니다. **긴 개념 제목을 통째로 문자열 사전으로 쓴 방법**의 적중이
낮다는 것이다. "카테고리 트리의 정체"는 사용자가 "카테고리 트리"라고만 말하면 안 잡힌다. 개념 지도는 정본과
근거 연결용으로 두고, 라우팅 어휘는 configure에서 사람이 짧은 별칭을 붙이는 쪽이 타당하다. 제안이지 입증은 아니다.

### 멈춤을 뒤집는 조건 (코덱스)

| 이런 증거가 나오면 | 그때 |
|---|---|
| 원문 대조한 실패 표본에서 기존 종류로 명확히 분류되는데 단순 표현 누락인 요청이 반복되고, 그 누락이 실제 라우팅에 필요함이 확인됨 | ① 사전 확장을 **별도 후속 실험**으로. 현 v0.1.0은 보존 |
| 표현 보완 뒤에도 의미·문맥 판별이 필요한 실패가 남고, 같은 미열람 라벨 집합에서 Rules+Jev가 Rules only보다 잘못된 라우팅을 줄이며 비용이 허용 범위 안 | ② Jev 도입 근거 |
| 실패 대부분이 비작업·맥락 부족 발화이거나 분류 개선이 실제 작업에 도움이 안 됨 | ③ 유지 |

증거를 만들려면 5판의 표본 절차(0-A 양쪽 원문 대조, STEP 2 20/20/10 라벨, NO_MATCH를 표현 누락/맥락 필요/분류 밖으로 분리)를
끝내면 된다. 합격선은 새 평가를 보기 전에 정한다. 같은 276건에서 고친 결과는 TUNED_ON_SEEN_CORPUS다.
