...
if [ "$META" -eq 0 ]; then
  echo "[driftguard] 개념 지도가 있는데 메타 줄이 없다 (개념 ${CONC}개)."
...
CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/driftguard"
...
echo "[driftguard] 개념 지도에 재확인 대상 ${STALE}건 (신선 ${FRESH}건). 자세히는 /drift"
